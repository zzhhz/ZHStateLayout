...
import com.zzh.sl.empty.RecyclerAdapterEmptyStrategy;

import java.util.HashSet;
import java.util.Set;


public class HStateLayout extends FrameLayout {
    ...
    public IStateView getErrorView() {
        if (mErrorView == null) {
            ...
            final int layoutId = getLayoutId(getContext(), getResources().getString(R.string.zh_state_layout_error_layout));
    ...
    public IStateView getEmptyView() {
        if (mEmptyView == null) {
            ...
            final int layoutId = getLayoutId(getContext(), getResources().getString(R.string.zh_state_layout_empty_layout));
    ...
    private final HViewListener<View> mContentListener = new HViewListener<View>() {
        ...
        protected void onUpdate(View view) {
            if (mEmptyStrategy != null) {
                ...
                final HStateEmptyStrategy.Result result = mEmptyStrategy.getResult();
                if (result == null)
    ...
    /**
     * 用{@link #setEmptyStrategy(HStateEmptyStrategy)}替代
     *
     * @param adapter
     */
    @Deprecated
